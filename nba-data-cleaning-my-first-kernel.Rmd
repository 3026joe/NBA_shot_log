---
title: "NBA Player Dataset"
author: "Joseph Dominic"
output:
    html_document:
        toc: yes
        theme: spacelab
...
We require only the data from the 2016-17 season, as the accuracy of stat recording is best for the most recent season, and the most relevant. We will also use the salaries of the 2017-18 season.

The remaining data will only be used for visualizations.
 ...
 * `GS`: Games where the player was on the starting line-up.  
...
* Are the Win Shares the most correlated to the salary of a player?

* Is there any significant difference in any of the main statistics between positions of the game? 
...
First, we removed 2 blank variables.
...
The salaries_2017 dataset doesn't contain any null value, as shown below.
...
Now we should think if those values should be assigned or else if we can drop them from the dataset.
...
The second case is important as it contains good players who played quite a few minutes.
...
Next, we replace the NAs in  `X3P.`.
...
These are players who didn't shoot any 3's, so we simply replace the value with 0.
...
#View(leveneTest(Salary.in..  ~ Pos, data = subset2))
```
...
```{r}
# homocedasticity test
#View(leveneTest(PPG  ~ Pos, data = subset2))
```
...
```{r}
# homocedasticity test
#View(leveneTest(APG  ~ Pos, data = subset2))
```
...
```{r}
#View(leveneTest(WS  ~ Pos, data = subset2))
```
...
View(subset %>% filter(Player %in% player_names))
# salaries in 2016-17
true_salaries <-
  salaries_2017 %>% 
